...
class ProfileView(tk.Frame):
    ...
    def build_profile_form(self):
        ...
        ttk.Entry(frame, textvariable=self.user_id, state="disabled").grid(row=0, column=1, padx=10, pady=10, sticky="ew")
    ...
    def save_profile(self):
        ...
        ok = self.users.update_profile(self.user_id.get(), updated)
        if ok:
    ...
    def change_password(self):
        old = str(self.old_pw_var.get().strip())
        new = str(self.new_pw_var.get().strip())
        confirm = str(self.confirm_pw_var.get().strip())
        ...
        ok = self.users.change_password(self.user_id.get(), old, new)
        if ok:
