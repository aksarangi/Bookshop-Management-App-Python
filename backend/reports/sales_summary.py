...
class SalesSummaryReport:
    ...
    def get_overview_summary(self):
        ...
        - total_authors
        - total_publishers
        - total_sales (number of orders)
        - today_revenue (sum of successful payments today)
        - low_stock (books where stock < 10)
        """

        conn = get_connection()
        if not conn:
            logger.error("DB connection failed in get_overview_summary()")
            return {
                "status": "error",
        ...
        try:
            cursor.execute("""
                SELECT 
                    -- Total books
                    (SELECT COUNT(*) FROM books) AS num_books,
            ...
            logger.error(f"Error fetching overview summary: {e}")
